...
export function Navbar() {
  return (
    ...
    >
      <Link to="/" className="font-body text-sm font-medium uppercase tracking-widest text-cream">
        MOE ✦
      </Link>
      ...
      <ul className="hidden items-center gap-10 md:flex">
        {links.map(({ label, href }, i) => (
          ...
          >
            {href.startsWith('/') ? (
              <Link
                to={href}
                className="text-cream-55 text-[13px] tracking-wide transition-colors duration-200 hover:text-cream"
              >
                {label}
              </Link>
            ) : (
        ...
        >
          <motion.a
            href="#contact"
            className="border-cream-10 rounded-full border px-5 py-2 text-[13px] tracking-wide text-cream"
